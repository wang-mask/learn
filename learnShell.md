...
## 3. 浏览文件命令
	...
	less filename（浏览文件全部内容，输入q退出浏览）
...
|N|低优先级进程。 一个低优先级进程，只有当其它高优先级进程被服务了之后，才会得到处理器时间。|
	...
	Ctrl-z   暂停前台任务
	kill pid    沙司进程（其实是给进程传递终止信号）
	kill [-signal] PID...    给进程发送指定信号，默认终止
|信号代码|英文缩写|意义|
|----|----|----|
|1|HUP|挂起（Hangup）。名字来源于很久以前，那时候终端机通过电话线和调制解调器连接到 远端的计算机。这个信号被用来告诉程序，控制的终端机已经“挂断”。 通过关闭一个终端会话，可以展示这个信号的作用。在当前终端运行的前台程序将会收到这个信号并终止。
